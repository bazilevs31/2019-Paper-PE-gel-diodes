...
%$\Pi_{osm}$, described in eq. \ref{eq:osmideal}.  In the simplest case of high temperature and weak electrostatics, the osmotic pressure of mobile ions can be expressed as a pressure of a uniform cloud of an ideal gas, with no electrostatics playing role.
\begin{equation}\label{eq:osmideal}
    \Pi^{osm}_{ideal} = k_B T \frac{N_{ci}}{V}=k_B T \frac{fN_{m}}{V},
    %= k_B T \frac{f N_m}{R_e^3} =  R T c_m f,
\end{equation}
...
%In the ideal case of a static network $c_2, c_1 = f_2, f_1$ the ionization degrees of the gel. It will be shown that at low temperatures, with low degrees of swelling and small boundary displacement the mechanical work is negligible and could be neglected $\Delta W_{mech} \sim 0$
